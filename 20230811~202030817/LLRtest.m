...
% binary_signal=zeros(frame_num,Tx_num*(1644*560-822*40),length(SNR_in_dB));
...
for count=1:length(SNR_in_dB)
    ...
    for frame=1:frame_num
        ...
        %demod
        data_dec_L_hat=qamdemod(data_mod_L_hat,QAM,'gray');%,'OutputType','approxllr');
        data_bin_L_hat=dec2bin(data_dec_L_hat,q_bit);
        data_bin_L_hat=data_bin_L_hat-'0';  %ASCII code char轉double
