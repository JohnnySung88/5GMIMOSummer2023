	...
	for(row=0;row<648;row++){
		...
		for(col=0;col<H_row_master_size[row];col++){
			...
			for(kcol = 0;kcol<H_row_master_size[row];kcol++){
				...
				if( fabs(S[kcol])<mini )
					mini = fabs(S[kcol]) - C_offset;
			}
			S[col] = ram;
			maxma = (mini>0)?mini:0;
			R[row][ H_row_master[row][col] ] = product*maxma;
...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[]){
	...
	C_offset		=*mxGetPr( C_offsetIN );
	mwSize dims[2] 	= {1296,5480};
