...
for count=1:length(SNR_in_dB)
    ...
    for frame=1:frame_num
        ...
        data_dec=randi([0,QAM-1], Tx_num*(1644*560-822*40)*code_rate  ,1); %只能傳送CODERATE的資訊量
        data_bin_dou= dec2bin(data_dec,q_bit)-'0'; %轉double
        data_bin_dou2=reshape(data_bin_dou.',Tx_num*(1644*560-822*40)*code_rate*q_bit,1);
        data_bin_Matrix= reshape(data_bin_dou2,648,5480);

        LDPC_bin_Matrix= mod(data_bin_Matrix.'* double(LDPC.G.x),2) ;
        ...
        for symbol=1:560
            if mod( (symbol - 3) , 14) == 0
                data_range = reshape(LDPC_mod(index:index+ 822*Tx_num-1),1, 822,Tx_num );
                LDPC_mod_data(:,symbol,:) = reshape([ data_range  ; DMRS_value * ones(1,822,Tx_num)],1644,2);
                index= index + 822*Tx_num;
            else
                LDPC_mod_data(:,symbol,:) = reshape(LDPC_mod(index:index+1644*Tx_num-1),1644,Tx_num );
                index= index + 1644*Tx_num;
            end
        end
        ...
        for symbol=1:560
            if mod( (symbol - 3) , 14) == 0
                data_mod_L_hat(index:index+ 822*Tx_num-1) = reshape(X_hat_L(1:2:1644,symbol,:), 822*Tx_num,1 );
                index= index + 822*Tx_num;
            else
                data_mod_L_hat(index:index+1644*Tx_num-1)= reshape(X_hat_L(1:1:1644,symbol,:),1644*Tx_num,1 );
                index= index + 1644*Tx_num;
            end
        end
        ...
        y_LLR=zeros(q_bit*length(data_mod_L_hat),1);
        ...
        BER=BER+sum(sum(data_bin_dou2~= data_bin_LLR),'all');
        BER_OMS=BER_OMS+sum(sum(data_bin_dou2~= data_bin_OMS),'all');
    end

    binary_LLR(1,count) =BER/(1644*560*q_bit*frame_num*Tx_num);
