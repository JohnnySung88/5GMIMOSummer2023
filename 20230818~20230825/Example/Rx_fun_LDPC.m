function [Time,Biterror,Capacity_sum,Rx1_SNR,Rx2_SNR,Lena_RGB] = Rx_fun_LDPC(frame_data,Rx_signal,DTinfo,CFO_ingore,iteration)
	Fs 	= frame_data.Fs;
	Tx	= frame_data.Tx;
	Rx  = frame_data.Rx;
	...
	y = syc(frame_data,y);
	%CFO估測(需多重路徑猜測) 背景運算
	f(1) = parfeval(backgroundPool,@CFO_EST,2,frame_data,y,Fs,CFO_ingore);
	%移除CP 0.072sec
