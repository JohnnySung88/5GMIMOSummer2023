...
LLR_part			 = reshape(LLR,LDPC_num,1296);
LLR_part			 = permute(LLR_part,[2,1]);

%NO LDPC decode(image)
bin_table	= 2 .^ (7:-1:0);
Lena_bin_hat = reshape(LLR_part(1:648,:).'<0,[],8);
Lena_bin_hat = Lena_bin_hat(1:Lena_size,:);
Lena_bin_hat = sum(Lena_bin_hat.*bin_table,2);%bin2dec
Lena_Csize	 = Lena_row*Lena_col;
Lena_RGB	 = zeros(Lena_row,Lena_col,3);
Lena_RGB(:,:,1) = reshape( Lena_bin_hat(             1:Lena_Csize  ) ,Lena_row,Lena_col);
Lena_RGB(:,:,2) = reshape( Lena_bin_hat(Lena_Csize  +1:Lena_Csize*2) ,Lena_row,Lena_col);
Lena_RGB(:,:,3) = reshape( Lena_bin_hat(Lena_Csize*2+1:Lena_Csize*3) ,Lena_row,Lena_col);
%imshow( uint8(Lena_RGB) );
%LDPC decode(image)
bin_table	= 2 .^ (7:-1:0);
LLR_MSA			  = LDPCMSAcustom(LLR_part,iteration,H_row_master,H_row_master_size,LDPC_num,0.5);
