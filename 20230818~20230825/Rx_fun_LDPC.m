function [Time,Biterror,Capacity_sum,Rx1_SNR,Rx2_SNR,Pict_RGB] = Rx_fun_LDPC(frame_data,Rx_signal,DTinfo,CFO_ignore,iteration)
	Fs 	= frame_data.Fs;
	Tx	= frame_data.Tx;
	Rx  = frame_data.Rx;
	...
	LLR_OMS			  	 = LDPC_OMS(LLR_part,iteration,frame_data.H_row_master,frame_data.H_row_master_size,5464,0.5);
	Pict_bin_hat 		 = reshape(LLR_OMS(1:648,:).'<0,[],8);
	%Time
	Time = toc;
	%LDPC decode(image)
	bin_table	= 2 .^ (7:-1:0);
