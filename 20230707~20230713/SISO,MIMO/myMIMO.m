...
for v=1:3
    ...
    parfor a=1:length(SNR_in_dB)
        ...
        for seperate = 1:data_num
            data = randi([0 QAM-1], Tx, 1);   % 隨機產生0~QAM-1 for QAM
            bin_data = de2bi(data, q_bit, 'left-msb');  % 將 0~QAM-1 轉為 binary
            X = qammod(data,QAM,'UnitAveragePower',true);   % 0~QAM-1 to complex (Modulation); remember to normalize
            H = (1/sqrt(2*Tx))*(randn(Rx,Tx) + 1j*randn(Rx,Tx));  % randn產生channel(注意正規化的問題)
            n = sqrt(No/2)*(randn(Rx,1) + 1j*randn(Rx,1));  % randn產生noise variance=No
