...
Complex2 ZF,Y,H;
double complex  y[1644][560][4]		={};
double complex  h[1644][560][4][4]	={};
double complex zf[1644][560][4]		={};
...
int HPos(int sc,int slot,int row,int col){
	return sc + 1644*slot + 1644*560*row + 1644*560*4*col;
}
...
void culcal(){
	...
	for(SC=0;SC<1644;SC++){
		for(slot=0;slot<560;slot++){
			...
			multip1 ( x ,inh,tep);
			for(i=0;i<Tx;i++){
				ZF.real[YPos(SC,slot,i)] = creal(x[i]);
				ZF.imag[YPos(SC,slot,i)] = cimag(x[i]);
			}
		}
	}
}
...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[]){
	...
	ZF_OUT	= mxCreateNumericArray(3,dims,mxDOUBLE_CLASS,mxCOMPLEX);
	ZF.real = mxGetPr( ZF_OUT );
	ZF.imag = mxGetPi( ZF_OUT );
	culcal();
}