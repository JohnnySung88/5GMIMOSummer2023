...
MSE_dB_LMMSE = zeros(length(SNR_weight),length(SNR_in_dB));

for i = 1:length(SNR_weight) % New loop for different SNR weights
    SNR_weights = SNR_weight(i);
    %LMMSE 權重矩陣
    W = make_W_S(window,SNR_weights);
    ...
    for a      = 1:length(SNR_in_dB)
        ...
        for frame = 1:frame_num	
            fprintf("SNR_weight: %d/%d \t SNR_in_dB : %d/%d \t frame : %d/%d\n", i, length(SNR_weight), a,length(SNR_in_dB),frame,frame_num);
            % Mod
        ...
        MSE_dB_LS(1,a)    = 10*log10( MSE_LS    / (822  * 40 * frame_num) );
        MSE_dB_LMMSE(i,a) = 10*log10( MSE_LMMSE / (1644 * 40 * frame_num) );
    end
end
...
xlabel('SNR (dB)');
ylabel('MSE (dB)');
legend(['LS MSE', arrayfun(@(x) ['LMMSE SNR=' num2str(x)], SNR_weight, 'UniformOutput', false)]); % Modify legend
